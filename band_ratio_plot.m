...
for i = 1:length(X_ratio)
    y_width(i) = band_width(nL,nR,tL,X_ratio(i)*tL,OA,OB);
end

%%

for i = 1:length(R_ratio)
    y_width_R(i) = band_width(nL,R_ratio(i)*nR,tL,tL,OA,OB);
end

%%
figure()
subplot(2,1,1)
scatter(X_ratio,y_width,'MarkerFaceColor',[0 0.4470 0.7410])
xlabel('T_{ratio}')
ylabel('Band width')
set(gca,'FontSize',20)
subplot(2,1,2)
scatter(X_ratio,y_width_R,'MarkerFaceColor',[0.9290 0.6940 0.1250])
xlabel('R_{ratio}')
ylabel('Band width')
set(gca,'FontSize',20)
